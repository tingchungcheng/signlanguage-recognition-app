...
import { getGuideCrop } from "../utils/guideCrop";
import {
  cropFromLandmarks,
  imageBase64ToModelInput,
  imageUriToModelInput,
} from "../utils/imageToModelInput";
...
class TfjsClassificationService implements ClassificationService {
  ...
  async initialize(): Promise<ModelStatus> {
    ...
    try {
      ...
      const bins = await Promise.all(
        [WEIGHT_BIN_1, WEIGHT_BIN_2].map(async (assetModule: number) => {
      ...
      this.status = "error";
      this.errorMessage =
        error instanceof Error ? error.message : "Failed to load ASL model.";
    }
  ...
  async classifyFrame(
    ...
    try {
      inputBuffer = await imageUriToModelInput(imageUri, crop, mirrorFrontCamera);
    } catch (uriError) {
      if (imageBase64 != null && imageBase64.length > 0) {
        inputBuffer = await imageBase64ToModelInput(imageBase64, crop, mirrorFrontCamera);
      } else {
        throw uriError;
      }
    }
...
export const classificationService: ClassificationService = new TfjsClassificationService();
